...
num_samples = 100000  # Scaled to 100k
data = []
labels = []
...
class AudioDataset(Dataset):
    def __init__(self, features, targets):
        self.features = torch.tensor(features, dtype=torch.float32).unsqueeze(1)  # Channel dim for CNN
...
train_loader = DataLoader(train_dataset, batch_size=128, shuffle=True)  # Larger for speed
test_loader = DataLoader(test_dataset, batch_size=128)

# CNN Model (dynamic flattened size)
...
for epoch in tqdm(range(epochs), desc="Training epochs"):
    model.train()
    for features, targets in tqdm(train_loader, desc=f"Epoch {epoch+1} batches", leave=False):  # Inner progress
        if torch.cuda.is_available():
